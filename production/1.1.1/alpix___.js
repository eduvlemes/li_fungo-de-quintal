...
//if(sessionStorage.getItem('testMode')){
    ...
    console.log(schedule.specialRules);
        ...
        $('#dt_entrega').datepicker({
            beforeShow: function(input, inst){
                if($('[name="forma_envio"]:checked').val() == undefined){
                    $('#apx_schedule legend.titulo').addClass('obs');
                    return false;
                }else{
            ...
            beforeShowDay: function(date){
                ...
                if($('[name="forma_envio"]:checked').val() != undefined){
                    ...
                    if(disabled_days && disabled_days.attributes && disabled_days.attributes.shipping_methods && disabled_days.attributes.shipping_methods.data && disabled_days.attributes.shipping_methods.data.find(el => parseInt(el.attributes.id_loja_integrada) == parseInt(schedule.settings.id_loja_integrada ))){
                        disabled_days = true
                    }
                    else{
                        disabled_days = false
                    ...
                    parseInt(date.getFullYear()) <= parseInt(validDate.getFullYear())){
                        return [false, ""];
                    }else{
                        if(available_days.includes(day)){
                            ...
                            if(disabled_days){
                                ...
                                $.each(availablePeriods.filter(el => el.day_of_week == day), function(k, periods){
                                    $.each(periods.periods, function(k, period){
                                        ...
                                        let limitDateTime = schedule.functions.calcHours(estimatedDateTime,period.schedule_until_hour || 0)
    ...
    $('body').on('change','[name="forma_envio"]',function(){
        window.ATIVAR_AGENDAMENTO = ($('[name="forma_envio"]:checked')?.closest('[data-nome]')?.attr('data-nome')?.toLowerCase().includes('correios') || $('[name="forma_envio"]:checked')?.closest('[data-nome]')?.attr('data-id') == "162020") ? false : true;
